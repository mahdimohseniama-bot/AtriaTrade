...
class DummyExchangeAdapter(BaseExchangeAdapter):
    ...
    def get_ticker(self, symbol: str) -> Ticker:
        ...
        return Ticker(symbol=symbol, bid=price - 0.5, ask=price + 0.5,
                      last_price=price, volume=1500.0)
    ...
    def place_order(self, symbol: str, side: str, order_type: str, quantity: float, price: float = 0.0) -> OrderResponse:
        ...
        return OrderResponse(order_id=f"dummy_{int(time.time())}", symbol=symbol,
                             status="FILLED", price=exec_price, quantity=quantity,
