...
class BinanceTestnetAdapter(BaseExchangeAdapter):
    ...
    def get_ticker(self, symbol: str) -> Ticker:
        price_resp = self.session.get(f"{self.base_url}/ticker/price",
                                      params={"symbol": symbol}, timeout=5)
        price_resp.raise_for_status()
        last_price = float(price_resp.json()["price"])

        book_resp = self.session.get(f"{self.base_url}/depth",
        ...
        return Ticker(symbol=symbol, bid=best_bid, ask=best_ask,
                      last_price=last_price, volume=0.0)
    ...
    def place_order(self, symbol: str, side: str, order_type: str, quantity: float, price: float = 0.0) -> OrderResponse:
        ...
        params = {"symbol": symbol, "side": side.upper(), "type": order_type.upper(),
                  "quantity": quantity, "timestamp": int(time.time() * 1000)}
        ...
        return OrderResponse(order_id=str(data.get("orderId")), symbol=data.get("symbol"),
                             status=data.get("status"), price=float(data.get("price") or 0.0),
                             quantity=float(data.get("origQty")),
                             filled_quantity=float(data.get("executedQty")), raw_data=data)
